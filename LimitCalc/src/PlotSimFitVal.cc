...
//------------------------------------------------------------------------------
RooRealVar p("p","p",0,-7,7);

//------------------------------------------------------------------------------
//   Declaring Helper datatype
//------------------------------------------------------------------------------
...
void PlotGenFit( const vector<string>& masslist )
{
   ...
   val::MakePullComparePlot( p1plot  , "param1" );
   val::MakePullComparePlot( p2plot  , "param2" );
...
val::PullResult val::PlotSingleGenFit( const std::string& masstag )
{
   ...
   while( 1 ){
      ...
      p = bkgpull; bkgset.add( RooArgSet(p) );
      p = sigpull; sigset.add( RooArgSet(p) );
      p = p1pull;  p1set.add( RooArgSet(p) );
      p = p2pull;  p2set.add( RooArgSet(p) );
      p = signorm ; signormset.add(RooArgSet(p));
   }

   ans.bkgpull = val::MakePullPlot( bkgset, masstag,  "bkg"    );
   ans.sigpull = val::MakePullPlot( sigset, masstag,  "sig"    );
   ans.p1pull  = val::MakePullPlot( p1set,  masstag, "param1" );
   ans.p2pull  = val::MakePullPlot( p2set,  masstag, "param2" );
   val::MakeNormPlot( signormset, masstag, "sig" );
...
//------------------------------------------------------------------------------
Parameter val::MakePullPlot( RooDataSet& set, const string& masstag, const string& tag )
{
...
Parameter val::MakeNormPlot( RooDataSet& set , const string& masstag, const string& tag )
{
   TCanvas c("c","c",DEFAULT_CANVAS_WIDTH,DEFAULT_CANVAS_HEIGHT);
   RooPlot* frame = p.frame();
   PlotOn( frame, &set );
   ...
   // Title Formats
   boost::format xtitlefmt("{%1%}_{fit} / {%1%}_{inj}");
   boost::format signalfmt("Sig. mass = %1% GeV");
   boost::format injectfmt("signal events %1% %2%");
...
void val::MakePullComparePlot( TGraph* graph , const string& tag )
{
   TCanvas c("c","c",DEFAULT_CANVAS_WIDTH,DEFAULT_CANVAS_HEIGHT);
   graph->Draw("AP");
   graph->SetTitle("");
