...
#include "TstarAnalysis/ModifiedHitFit/interface/Constrained_Top.h"
...
bool Constrained_Top_Args::do_topgluon_flag () const
//
// Purpose: Return the do_topgluon_flag parameter.
//          See the header for documentation.
//
{
     ...
     _constrainer ( args.fourvec_constrainer_args() )
{
   ...
   sprintf ( buf, "(%d %d %d %d) = (%d %d %d %d)",
         nu_label, lepton_label, lepb_label, lepg_label,
         hadw1_label, hadw2_label, hadb_label, hadg_label
   );
   _constrainer.add_constraint ( buf );
