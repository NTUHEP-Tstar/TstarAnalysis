...
main( int argc, char const* argv[] )
{
  ...
  for( ev.toBegin(); !ev.atEnd(); ++ev ){
    ...
    for( const auto& jet : jethandle.ref() ){
      cout << "Original momentum:" << mgr::GetLorentzVector( jet, "" ).Pt() << endl;
      cout << "Smeared  momentum:" << mgr::GetLorentzVector( jet, "ResP4" ).Pt() << endl;
    }

    cout << "\n\nMuon info =========== \n" << endl;

    for( const auto& muon : muonhandle.ref() ){
      cout << "Original momentum:" << mgr::GetLorentzVector( muon, "" ).Pt() << endl;
      cout << "Trigger  momentum:" << mgr::GetLorentzVector( muon, "TrigP4" ).Pt() << endl;
    }

    cout << "\n\nElectron info =========== \n" << endl;

    for( const auto& elec : elechandle.ref() ){
      cout << "Original momentum:" << mgr::GetLorentzVector( elec, "" ).Pt() << endl;
      cout << "Trigger  momentum:" << mgr::GetLorentzVector( elec, "TrigP4" ).Pt() << endl;
    }
  }
