...
bool JetProducer::IsSelectedJet( const pat::Jet& jet, const bool isdata ) const
{
   ...
   for( const auto& mu : *_muonHandle ) {
      const TLorentzVector mup4 = GetLorentzVector( mu , "" ) ; // getting default four momentum
      if( jetp4.DeltaR(mup4) < 0.4 ) { return false;  }
   }
   for( const auto& el : *_electronHandle ){
      const TLorentzVector elp4 = GetLorentzVector( el , "" ) ; // getting default four momentum
...
void JetProducer::AddJetVariables(
   pat::Jet& jet,
   const bool is_data
)
{
   ...
   if( is_data ) { return ; } // Early exit for data jets, no need to add new jet for data

   const TLorentzVector resjet = IsWellMatched( jet ) ? MakeScaled( jet ) : MakeSmeared( jet );
   AddLorentzVector( jet, resjet, "ResP4" );
}

/*****************************************************************************/

bool JetProducer::IsWellMatched( const pat::Jet& jet ) const
{
...
TLorentzVector JetProducer::MakeScaled( const pat::Jet& jet ) const
{
   const TLorentzVector jetp4( jet.px(), jet.py(), jet.pz(), jet.energy() );
   const TLorentzVector genp4( jet.genJet()->px(), jet.genJet()->py(), jet.genJet()->pz(), jet.genJet()->energy() );
...
union MyConv{ float thefloat ; unsigned theint ; };
TLorentzVector JetProducer::MakeSmeared( const pat::Jet& jet ) const
{
   static MyConv seeder;
   static TRandom3 ran;
...
double JetProducer::GetJetPtRes( const pat::Jet& jet ) const
{
   JME::JetParameters jetparm;
   jetparm.setJetPt( jet.pt() ).setJetEta( jet.eta() ).setRho(*_rhoHandle);
...
double JetProducer::GetJetResScale( const pat::Jet& jet ) const
{
   JME::JetParameters jetparm;
   jetparm.setJetPt( jet.pt() ).setJetEta( jet.eta() ).setRho(*_rhoHandle);
