...
cmsRun {0}/RunSequence/cmsrun/run_supselection.py maxEvents=-1 sample={1} output={2}
"""
...
def main():
    ...
    with open(opt.input) as f:
        ...
        for dataset in dataset_list:
            ...
            glob_query = my_name.GetStoreGlob(
                'massreco', dataset,  opt.mode)
            ...
            for index, file_list in enumerate(file_chunks):
                ...
                output = my_name.GetStoreFile(
                    'supsel', dataset, opt.mode, index)
                script_file_name = my_name.GetScriptFile(
                    'supsel', dataset, opt.mode, index)
