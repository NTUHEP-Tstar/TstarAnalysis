...
HitFitter::RunPermutations()
{
   ...
   do {
      ...
      for( const auto jet : _btagJetList ){
         AddHitFitJet( hitFitEvent, jet, jet_type_list[i] );
         ++i;
      }
...
// ------------------------------------------------------------------------------
void
HitFitter::SetMET( const pat::MET* met )
{
   _met = met;
}

void
HitFitter::SetMuon( const pat::Muon* mu )
{
   _muon = mu;
}

void
HitFitter::SetElectron( const pat::Electron* elec )
{
   _elec = elec;
}

void
HitFitter::AddBTagJet( const pat::Jet* jet  )
{
...
void
HitFitter::AddLightJet( const pat::Jet* jet )
{
...
void
HitFitter::SetHitFitTemplate( hitfit::Lepjets_Event& hitFitEvent ) const
{
...
void
HitFitter::AddHitFitJet( hitfit::Lepjets_Event& hitFitEvent, const pat::Jet* jet, const int type ) const
{
...
bool
HitFitter::CheckBTagOrder( const std::vector<int>& jet_type_list ) const
{
