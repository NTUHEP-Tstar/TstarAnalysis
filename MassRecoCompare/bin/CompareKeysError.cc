...
main( int argc, char* argv[] )
{
  ...
  *   Options definition
  *******************************************************************************/
  opt::options_description desc( "Options for comparing Keys Pdf results with errors" );
  desc.add_options()
    ( "mass,m", opt::value<int>(), "Masspoint to run" )
    ( "compare,x", opt::value<string>()->default_value("ChiSq6jet2b"), "Which reconstruction methods to use" )
  ;
  ...
  *******************************************************************************/
  ...
  KeysErrMgr* compmgr = new KeysErrMgr( name, ltxname );
