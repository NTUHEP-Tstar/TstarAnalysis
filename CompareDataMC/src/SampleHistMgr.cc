...
SampleHistMgr::fill_histograms( SampleMgr& sample )
{
   ...
   for( const auto& file : sample.GlobbedFileList() ){
      ...
      for( ev.toBegin(); !ev.atEnd(); ++ev, ++i ){
         ...
         Hist( "VtxNumNW" )->Fill( vtxHandle->size(),
            total_weight/pileup_weight );
         ...
         if( chisqHandle->ChiSquare() > 0 ){// Only sotring physical results
            ...
            Hist( "LepGluonPt" )->Fill(
               chisqHandle->LeptonicGluon().Pt(), total_weight  );
            Hist( "HadGluonPt" )->Fill(
               chisqHandle->HadronicGluon().Pt(), total_weight  );

            if( chisqHandle->TstarMass() > 350 ){
               Hist( "TstarZoom" )->Fill( chisqHandle->TstarMass(),
                  total_weight );
            }
         }
      }
