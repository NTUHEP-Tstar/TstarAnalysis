...
SampleHistMgr::define_hist()
{
   ...
   AddHist( "VtxNumUp",          "Number of primary vertex (#sigma_{min} = 71.2)", "",          50,   0,     50 );
   AddHist( "VtxNumDown",        "Number of primary vertex (#sigma_{min} = 62.0)", "",          50,   0,     50 );
   AddHist( "VtxNumNW",          "Number of primary vertex(no weighting)",         "",          50,   0,     50 );

   // Pile up effects
   AddHist( "Jet1Pt_PuUp",       "First Leading Jet p_{T} (#sigma_{min}=71.2)",    "GeV/c",     60,  30,   1000 );
   AddHist( "Jet1Pt_PuDown",     "First Leading Jet p_{T} (#sigma_{min}=62.0)",    "GeV/c",     60,  30,   1000 );
   AddHist( "JetNum_PuUp",       "M_{t+g} (#sigma_{min} = 71.2)",                  "GeV/c^{2}", 50,   0,   2000 );
   AddHist( "JetNum_PuDown",     "M_{t+g} (#sigma_{min} = 62.0)",                  "GeV/c^{2}", 50,   0,   2000 );
   AddHist( "TstarMass_PuUp",    "M_{t+g} (#sigma_{min} = 71.2)",                  "GeV/c^{2}", 50,   0,   2000 );
   AddHist( "TstarMass_PuDown",  "M_{t+g} (#sigma_{min} = 62.0)",                  "GeV/c^{2}", 50,   0,   2000 );

   // Effects of TopPtWeight
   AddHist( "MET_NoTopPt",       "Missing p_{T} (w/o top p_{T} weights)",          "GeV/c",     50,   0,    500 );
   ...
   AddHist( "LepNonePt",         "selection lepton p_{T} (no electron weights)",   "GeV/c",     48,  20,    500 );
   AddHist( "LepNoneEta",        "selection lepton #eta (no electron weight)",     "",          75,   -2.5, 5.0 );
   AddHist( "LepMediumPt",       "medium selection lepton p_{T}",                  "GeV/c",     48,  20,    500 );
   AddHist( "LepMediumEta",      "medium selection lepton #eta",                   "",          75,   -2.5, 5.0 );
   AddHist( "LepTightPt",        "tight selection lepton p_{T}",                   "GeV/c",     48,  20,    500 );
   AddHist( "LepTightEta",       "tight selection lepton #eta",                    "",          75,   -2.5, 5.0 );
...
SampleHistMgr::fill_histograms( SampleMgr& sample )
{
   ...
   for( const auto& file : sample.GlobbedFileList() ){
      ...
      for( ev.toBegin(); !ev.atEnd(); ++ev, ++i ){
         ...
         const double puweight_up         = GetPileupWeight71260( ev );
         const double puweight_down       = GetPileupWeight62000( ev );
         const double elecloosecutweight  = GetElectronCutWeight( ev );
         const double elecmediumcutweight = GetElectronMediumCutWeight( ev );
         const double electightcutweight  = GetElectronTightCutWeight( ev );
         ...
         for( const auto& el : *electronHandle ){
            ...
            Hist( "LepNonePt" )->Fill( el.pt(), total_weight / elecloosecutweight );
            Hist( "LepNoneEta" )->Fill( el.eta(), total_weight / elecloosecutweight );
            if( el.userInt( "passMedium" ) ){
               Hist( "LepMediumPt" )->Fill( el.pt(), total_weight * elecmediumcutweight / elecloosecutweight );
               Hist( "LepMediumEta" )->Fill( el.eta(), total_weight * elecmediumcutweight / elecloosecutweight );
            }
            if( el.userInt( "passTight" ) ){
               Hist( "LepTightPt" )->Fill( el.pt(), total_weight * electightcutweight / elecloosecutweight );
               Hist( "LepTightEta" )->Fill( el.eta(), total_weight * electightcutweight / elecloosecutweight );
            }
         }
